...
export default function ExploreDoctors({ initialPath = 'search', citySlug = '', specialitySlug = '' }) {
  ...
  return <><Seo title={`${resultTitle} | mediTrust`} description={seoDescription} structuredData={structuredData} /><Header /><main className="discovery-page"><section className="discovery-top"><div className="discovery-shell"><aside className="discovery-intro"><p>Doctor discovery</p><h1>Find the right<br /><em>starting point.</em></h1><ol><li><span>01</span>Choose how you want to search</li><li><span>02</span>Refine when you are ready</li><li><span>03</span>Review the details</li></ol></aside><div className="discovery-panel"><div className="discovery-tabs" role="tablist"><button type="button" onClick={() => setPath('search')} className={path === 'search' ? 'is-active' : ''}><Route size={16} /> I know the speciality</button><button type="button" onClick={() => setPath('guide')} className={path === 'guide' ? 'is-active' : ''}><Sparkles size={16} /> I’m not sure</button></div>{catalogError ? <p className="discovery-error">{catalogError}</p> : path === 'search' ? <DiscoveryForm values={selections} onChange={changeSelections} onSubmit={(event) => { event.preventDefault(); setError(''); setShowResults(true) }} locations={catalog.locations} specialities={catalog.specialities} onLocate={locate} locating={locating} locationError={locationError} /> : <SpecialityGuide onChoose={chooseGuidance} />}</div></div></section>{showResults && <section className="discovery-results"><div className="discovery-results-inner"><header><p>Results</p><h2>{resultTitle}</h2></header><ResultControls filters={filters} onChange={(next) => { setError(''); setFilters((current) => ({ ...current, ...next })) }} onClear={() => { setError(''); setFilters(initialFilters) }} />{isLoading || routeLoading ? <div className="grid gap-4 py-7"><div className="h-35 animate-pulse rounded-2xl bg-sand" /><div className="h-35 animate-pulse rounded-2xl bg-sand" /></div> : error ? <p className="py-10 text-center text-coral">{error}</p> : results.length ? <div>{results.map((doctor) => <DoctorCard key={doctor.id} doctor={doctor} />)}</div> : <div className="discovery-empty"><h3>No profiles match this search.</h3><p>Try a different name, care need, location, or speciality.</p></div>}</div></section>}</main><Footer /></>
}
