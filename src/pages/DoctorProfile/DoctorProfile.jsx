...
export default function DoctorProfile() {
  ...
  const directionsUrl = primaryPractice?.mapUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(doctor.address || primaryPractice?.address || `${doctor.primaryLocation.city}, ${doctor.primaryLocation.state}`)}`
  const lastChecked = doctor.verification.lastVerifiedAt ? new Date(doctor.verification.lastVerifiedAt).toLocaleDateString() : doctor.verification.status === 'VERIFIED' ? 'Verification date unavailable' : 'Not yet verified'
